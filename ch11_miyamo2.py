...
class InitData():
    ...
    def load_data(self, fname):
        ...
        for l in f.readlines():
            ...
            # print(sample)
            label = int(sample[0])                  #ラベル=教師データ(0/1)
            target.append(label)                    #target=ラベル(0/1)列
            # document_list.append(sample[1].split()) #単語分割して文書リストに追加
            document_list.append(self.to_words(sample[1])) #単語分割して文書リストに追加
            comment.append(sample[1])               #comment=文章
            word.append(self.to_words(sample[1]))   #単語分割してwordリストに追加
            # print(sample[1])
        ...
        #単語辞書を作成
        dictionary = {}   
        dictionary = corpora.Dictionary(document_list)     #単語列を辞書に追加
        dictionary.filter_extremes(no_below=0, no_above=100) 
        # no_below: 使われている文書がno_below個以下の単語を無視
        # no_above: 使われてる文章の割合がno_above以上の場合無視
        
        #文書のベクトル化
        for document in document_list:  #単語分割された各文書
            tmp = dictionary.doc2bow(document) #文書をBoW表現=辞書内の単語IDの頻度のタプル列
            vec = list(matutils.corpus2dense([tmp], num_terms=len(dictionary)).T[0]) #辞書内全単語と頻度のリスト=文章の特徴ベクトル
            source.append(vec) #文章の特徴ベクトルをsourceに追加


        dataset = {}
        dataset['source'] = np.array(source) #入力特徴ベクトル
        dataset['target'] = np.array(target) #教師データ
        dataset['comment'] = comment;        #文書
        dataset['word'] = word;              #単語分割した文書
        print("------------")
        print("source:", dataset['source'])
        print("target:", dataset['target'])
        print("comment:", dataset['comment'])
